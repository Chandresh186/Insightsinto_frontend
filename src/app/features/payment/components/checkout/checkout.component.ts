...
export class CheckoutComponent {
  ...
  public appliedPromoCodeId: any;
  public appliedPromoCode: any;
  ...
  handleInput(event: Event): void {
    ...
    const formattedValue = (inputField.value = letters.toUpperCase() + numbers);

    // ✅ Update the form control's value — not just the input field
    this.couponForm
      .get('couponCode')
  ...
  applyCoupon() {
    ...
    console.log(this.couponForm.get('couponCode')?.value.toUpperCase());

    this.loading = true; // Set loading state
    this.couponService
      .getCouponByCode(this.couponForm.get('couponCode')?.value.toUpperCase())
      .pipe(
        tap((response) => {
          ...
          if (
            response.data.code ===
            this.couponForm.get('couponCode')?.value.toUpperCase()
          ) {
            // // this.getNewOrderId();
            // this.isValid = true;
            ...
            if (
              ...
              response.data.productId === this.selectedProduct.id
            ) {
              this.isValid = true;
              this.couponMessage = 'Coupon applied successfully!';
              // Save promo details
              this.appliedPromoCodeId = response.data.id;
              // this.discountAmount = response.data.discountAmount;
              this.appliedPromoCode = response.data.code;
  ...
  createOrder() {
    ...
    this.paymentService
      .createOrder(paymentOrderData)
      .pipe(
        tap((response: any) => {
          this.payWithRazorpay(
            response && response.data && response.data.orderId
          );
        }),
        catchError((error) => {
