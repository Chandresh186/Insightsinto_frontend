...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TableComponent, RouterLink, ngbootstrapModule, FormsModule, ReactiveFormsModule],
...
export class UserListComponent implements OnInit {
  ...
  onDetails(val: any) {
      ...
      this.authService
        ...
        .pipe(
          tap((response: any) => {
            ...
            this.showColumns  = this.generateTableHeaders(response.data.map(({ id,userId,roleId, ...rest }: any) => rest));
    ...
    open(content: TemplateRef<any>) {
      this.modalService.open(content, { scrollable: true, ariaLabelledBy: 'create-user' }).result.then(
        (result) => {
