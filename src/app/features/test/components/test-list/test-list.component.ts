...
export class TestListComponent implements OnInit {
  ...
  atLeastOneQuestionValidator(): ValidatorFn {
    return (group: AbstractControl): ValidationErrors | null => {
      ...
      const hasAtLeastOne = [easy, medium, hard].some(
        (val) => !!val && val > 0
      );
  ...
  calculateTotalQuestions() {
    ...
    const easyQuestions =
      Number(this.questionForm.get('easyQuestions')?.value) || 0;
    const mediumQuestions =
      Number(this.questionForm.get('mediumQuestions')?.value) || 0;
    const hardQuestions =
      Number(this.questionForm.get('hardQuestions')?.value) || 0;
  ...
  addCategoryChip(option: Category): void {
    if (this.testForm.categoryIds.indexOf(option) === -1) {
      // Avoid duplicates
  ...
  selectCatOption(option: Category): void {
    ...
    this.GeneratedQuestions = this.AccordionData.filter(
      (res: any) => res.categoryName === option.catName
    );
    console.log(this.GeneratedQuestions);
  ...
  subscribeToChanges() {
    const easyQuestionsSub: any = this.questionForm
      .get('easyQuestions')
    ...
    const mediumQuestionsSub: any = this.questionForm
      .get('mediumQuestions')
      ?.valueChanges.pipe(debounceTime(500))
      .subscribe(() => {
        this.calculateTotalQuestions();
    ...
    const hardQuestionsSub: any = this.questionForm
      .get('hardQuestions')
      ?.valueChanges.pipe(debounceTime(500))
      .subscribe(() => {
        this.calculateTotalQuestions();
  ...
  genQuestions() {
    const reqBody: any = {
      ...
      numberOfQuestionsMedium:
        this.questionForm.get('mediumQuestions')?.value || 0,
  ...
  addQuesToTest() {
    const storedIds = new Set(
      (this.StoredQuestions || []).map((q: any) => q.id)
    );
    const newQuestions = (this.AccordionData || []).filter(
      (q: any) => !storedIds.has(q.id)
    );
  ...
  EditTestseries() {
    ...
    this.testSeriesService
      .updateTestSeries(this.testSeriesDetails.id, reqBody)
      .pipe(
        tap((response: any) => {}),
        catchError((error) => {
